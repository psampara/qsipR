...
```{r, fig.cap=c("Sequin scaling regression plot"), echo = F, warning = F}
knitr::include_graphics("./mock_input_data/12C_rep_1_fraction_1.pdf")
```
...
```{r, fig.cap=c("Atom fraction excess with bootstrapped onfidence intervals to identify incorporators"), echo = F, warning = F}
knitr::include_graphics("./mock_input_data/atom_fration_excess.pdf")
```
...
```{r, fig.cap=c("mean abundance across buoyant density gradient"),echo = F, warning = F}
knitr::include_graphics("mock_input_data/mean_abundance_NC_000964.3.pdf")
```
